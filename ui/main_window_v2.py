...
class MainWindowV2(QMainWindow):
  ...
  def _force_reboot_device(self, device_ip: str, device_name: str):
    ...
    if reply == QMessageBox.StandardButton.Yes:
      ...
      self.status_bar.showMessage(f" Riavvio in corso per {device_name}...", 30000)
  ...
  def _on_reboot_progress(self, message: str):
    ...
    self.status_bar.showMessage(f" {message}", 30000)
  ...
  def _force_check_device(self, device_ip: str):
    ...
    try:
      ...
      if device_id:
        ...
        self.status_bar.showMessage(f"Check forzato per {device_ip}", 3000)
  ...
  def _refresh_data(self):
    ...
    try:
      ...
      self.status_bar.showMessage(
        f"Data aggiornati - {device_count} dispositivi ricaricati e check forzato",
        4000
      )
    except Exception as e:
      logger.error(f"Error refreshing data: {e}", exc_info=True)
      self.status_bar.showMessage(f"Errore aggiornamento: {str(e)}", 3000)
  ...
  def _check_now(self):
    ...
    try:
      ...
      self.status_bar.showMessage(" Esecuzione check istantaneo su tutti i dispositivi...", 30000)
  ...
  def _send_check_now_report(self):
    ...
    try:
      ...
      if success:
        ...
        self.status_bar.showMessage(f"Check completato e email inviata: {message}", 10000)
        QMessageBox.information(
          self,
          "Report Inviato",
          f"Check istantaneo completato!\n\n{message}\n\n"
          "L'email di report è stata inviata con tutti i dettagli:\n"
        ...
        self.status_bar.showMessage(f"⚠ Check completato ma email non inviata: {message}", 10000)
        QMessageBox.warning(
          self,
          "Email Non Inviata",
          f"Check completato ma si è verificato un errore nell'invio dell'email:\n\n{message}"
        )
    ...
    try:
      ...
      if self.aggregated_email_service.should_send_email():
        ...
        if success:
          ...
          self.status_bar.showMessage(f"Errore invio email: {message}", 10000)
      else:
  ...
  def _test_ssh_connectivity(self):
    ...
    try:
      ...
      if success:
        ...
        logger.info(f"Auto-recovery is ready with credentials: {self.ssh_config.get('username')}")
      else:
        logger.warning(f"SSH connectivity test FAILED: {message}")
        logger.warning("Auto-recovery may not work. Check SSH credentials in config.json")
